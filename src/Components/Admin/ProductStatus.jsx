...
export default function ProductStatus() {
  ...
  const getAllUserAndProducts = async () => {
    try {
      const res = await axios.post("https://ecommercebackend02.onrender.com/api/v1/user/product-status", { id: currentUser.data._id });
  ...
  const handleUpdateStatus = async (productId, newStatus) => {
    try {
      const res = await axios.post("https://ecommercebackend02.onrender.com/api/v1/user/updateStatus", {
        productId, 
        newStatus, 
      });
