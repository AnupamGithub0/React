...
import { removeFromCart, successfullyOrders } from "../redux/userSlice";
...
export default function CartCom() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://ecommercebackend02.onrender.com/api/v1/user/payment",
        { amount: totalAmount }
      );
  ...
  const verifyPayment = async (orderId) => {
    try {
      const res = await axios.post("https://ecommercebackend02.onrender.com/api/v1/user/verify", {
        orderId: orderId,
      });
      ...
      if (res.data[0].payment_status === "SUCCESS") {
        ...
        const saveRes = await axios.post(
          "https://ecommercebackend02.onrender.com/v1/user/save-payment",
          { payments: paymentData }
        );

        if (saveRes.data.success === true) {
          navigate("/user/orders");
          dispatch(successfullyOrders(saveRes.data));
        }
      }
    } catch (error) {
