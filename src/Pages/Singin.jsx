...
//  const {currentUser} = useSelector((state)=>state.user)
  ...
  const handleSignup = async(e)=>{
    e.preventDefault()
    try {
      const res = await axios.post("https://ecommercebackend02.onrender.com/api/v1/user/signup",{profileImage,userName,email,password})
      setLoading(true)
  ...
  const handleSignin = async(e)=>{
    ...
    setLoading(true)
    try {
      const res = await axios.post("https://ecommercebackend02.onrender.com/api/v1/user/signin",{email,password})
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      axios
        .post("https://ecommercebackend02.onrender.com/api/v1/user/google", userData)
        .then((result) => {
            ...
            <div className="bg-white p-8 rounded-lg shadow-lg md:max-w-[330px] max-w-[300px] w-full">
              ...
              {isLogin ? (
                <div>
                  ...
                  <button onClick={handleGoogleAuth} className="w-full py-2 px-4 rounded flex items-center justify-center border-2 mt-4 hover:bg-slate-200 transition-all focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-opacity-50">
                    <svg
                  ...
                  <Link to={"/forgot-password"} className="text-blue-500 hover:underline mt-3">Forgot password</Link>
                </div>
              ) : (
                <form onSubmit={handleSignup} className="w-full flex items-center justify-center flex-col">
                  <div className="relative overflow-hidden w-24 h-24 bg-transparent border-2 mb-3 rounded-full">
                    <label>
                      {
                        profileImage ? (<img src={profileImage} alt="" className="w-full h-full" />):(<div src={""} alt="" className="w-full h-full bg-gray-300" />)
                      }
                      {
                        profileImage ? (<div className="text-xs overflow-hidden bg-opacity-80 bg-slate-200 cursor-pointer text-center absolute top-10 left-2">
                          
                        </div>):(<div className="text-xs overflow-hidden bg-opacity-80 bg-slate-200 cursor-pointer text-center absolute top-10 left-2">
                        Upload Photo
                      </div>)
                      }
                      <input type="file" onChange={handleProfileImage} required className="hidden" />
                    </label>
                  </div>
