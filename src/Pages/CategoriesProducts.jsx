  ...
  const fetchingCategoriesProduct = async () => {
    try {
      const res = await axios.get(
        `https://ecommercebackend02.onrender.com/api/v1/product/all-products/${params.category}`
      );
      ...
      <div className="w-full md:w-[1100px] m-auto">
        ...
        <div className="flex flex-wrap justify-center">
          {categoriesProducts.map((product) => {
            ...
            ? ((product.costPrice - product.sellPrice)/product.costPrice*100):(<h1>0</h1>)
            ...
            return (
              ...
              >
                <Link to={`/product/${product._id}`}>
                  <img src={product.productImages} alt="" className="h-[50%] w-full object-full" />
                </Link>
                <div className="p-2">
                  <h1 className="text-sm p-1 bg-red-700 text-white w-fit rounded-full mt-1">
                    STOCK {product.stock}
                  </h1>
                  <p className="font-thin text-[15px]">{product.productName}</p>
                  ...
                  <div className="text-center mt-5">
                    <button onClick={() => handleAddToCart(product)} className="p-1 bg-black text-white hover:bg-red-600">
                      ADD TO CART
                    </button>
                  </div>
                </div>
